...
export default function KitchenDisplay({ orders, onUpdateStatus }: KitchenDisplayProps) {
  ...
  const handleStatusChange = async (orderId: string, currentStatus: OrderStatus) => {
    ...
    if (currentStatus === 'pending' || currentStatus === 'confirmed') {
      ...
      // In KDS, we might not want kitchen staff to do anything after ready, 
      // but let's allow them to revert or something? No, just keep it simple.
  ...
  if (kitchenOrders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">No hay pedidos pendientes por preparar.</p>
      </div>
    );
  }
